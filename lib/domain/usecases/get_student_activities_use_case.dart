...
class GetStudentActivitiesUseCase extends BaseUseCase<List<StudentActivityItemApiModel>?>{
  ...
  Future<AppResult<List<StudentActivityItemApiModel>?>> execute(GetStudentActivitiesRequest request) async {
    return call(() async {
      var items =  await _repository.getStudentActivities(request);
