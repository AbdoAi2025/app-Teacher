...
class GroupsRepository {
  ...
  Future<AddGroupResponse?> addGroup(AddGroupRequest request) async {
    Response response = await ApiService.getInstance().post(EndPoints.addGroup , data: request.toJson());
  ...
  Future<AddGroupResponse?> updateGroup(UpdateGroupRequest request) async {
    Response response = await ApiService.getInstance().put(EndPoints.updateGroup , data: request.toJson());
    AddGroupResponse responseResult = AddGroupResponse.fromJson(response.data);
    return responseResult;
  }

  Future<GetGroupDetailsResponse?> getGroupDetails(String id) async {
    Response response = await ApiService.getInstance().get("${EndPoints.getGroupDetails}/$id" );
    GetGroupDetailsResponse responseResult = GetGroupDetailsResponse.fromJson(response.data);
    return responseResult;
  }

  Future<dynamic> deleteGroup(String id) async {
    return await ApiService.getInstance().delete("${EndPoints.deleteGroup}/$id" );;
  }

  Future<List<Group>> fetchGroups() async {
    try {
      Response response = await ApiService.getInstance().get(EndPoints.getMyGroups);

      GetMyGroupsResponse responseResult = GetMyGroupsResponse.fromJson(response.data);
